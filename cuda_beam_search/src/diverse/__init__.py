...
try:
    from .cuda_beam_search import process_diverse_beam_search as process_diverse_beam_search_cuda
    CUDA_AVAILABLE = True
except ImportError:
...
class DiverseBeamSearchScorer(BeamSearchScorer):
    ...
    def process(
        ...
        if CUDA_AVAILABLE and input_ids.is_cuda:
            next_beam_scores, next_beam_tokens, next_beam_indices = process_diverse_beam_search_cuda(
                input_ids,
                next_scores,
                next_tokens,
                next_indices,
...
class CUDADiverseBeamSearchScorer(DiverseBeamSearchScorer):
    def process(
        self,
        input_ids: torch.LongTensor,
        next_scores: torch.FloatTensor,
